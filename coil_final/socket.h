...
typedef enum ConnectionType {
	TCP=2,//makes for easier debugging in case it gets mixed up with sockettype for some reason
	UDP
};
...
class MySocket {
	...
	int socketStart() {
		...
		//if we somehow didn't trigger any of these, we have a problem
		return -1;
	}
...
public:
	...
	int GetData(char* buf) {
		...
		if (connectionType == TCP && bTCPConnect == true) {
			...
			recsize = recvfrom(ConnectionSocket, Buffer, MaxSize, 0, (struct sockaddr*)&SvrAddr, (socklen_t*) & AddrLen);
		}
		else if (connectionType == UDP && mySocket == SERVER) {
			recsize = recvfrom(ConnectionSocket, Buffer, MaxSize, 0, (struct sockaddr*)&CliAddr, (socklen_t*) & AddrLen);
		}
